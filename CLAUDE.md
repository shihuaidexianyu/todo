...
Task rows in lists are wrapped in `SwipeableTaskRow` (M3 `SwipeToDismissBox`): the action per direction is user-configurable in 设置 → 滑动手势 (`swipe_right` / `swipe_left` prefs; values 完成或恢复 / 安排到明天 / 关闭； defaults right=完成或恢复， left=安排到明天 via `TodoViewModel.scheduleTomorrow` — quiet, tags/reminder untouched). 安排到明天 never fires on an already-completed task and 关闭 disarms the direction (row drags but snaps back with no badge). `confirmValueChange` always returns false for actions — rows leave via the data change, never via dismissal — and callbacks plus the configured actions go through `rememberUpdatedState` so a swipe never acts on a stale `revision` or stale setting. The reveal is a floating circular icon badge (44dp, no text labels): complete = filled `primary` circle, reschedule = `surfaceContainerHigh`; its alpha/scale ramp with `state.progress * 2` so it's fully grown exactly when the 50% positional threshold arms the action (`progress` ≈ 0.5 there), and crossing the threshold on an applicable direction fires a `KEYBOARD_TAP` haptic. The background is `clearAndSetSemantics`-clean so TalkBack never announces it. Today's empty state becomes a celebration ("今天的都完成了。") when the list is empty because everything was completed today.
