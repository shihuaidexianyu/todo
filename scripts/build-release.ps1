...
try {
    ...
    New-Item -ItemType Directory -Force -Path '.signing','artifacts/release-validation/1.2.1' | Out-Null
    ...
    Copy-Item -LiteralPath app/build/outputs/apk/release/app-release.apk -Destination artifacts/todo-1.2.1-release.apk
    Copy-Item -LiteralPath app/build/outputs/bundle/release/app-release.aab -Destination artifacts/todo-1.2.1-release.aab
    Copy-Item -LiteralPath app/build/outputs/mapping/release/mapping.txt -Destination artifacts/release-validation/1.2.1/mapping.txt
    $taskSigner = Join-Path $taskSdk 'build-tools\36.0.0\apksigner.bat'
    & $taskSigner verify --verbose --print-certs artifacts/todo-1.2.1-release.apk | Tee-Object -FilePath artifacts/release-validation/1.2.1/signature.txt
    if ($LASTEXITCODE -ne 0) { throw 'APK 签名校验失败。' }
    Get-FileHash -Algorithm SHA256 -LiteralPath artifacts/todo-1.2.1-release.apk,artifacts/todo-1.2.1-release.aab | Format-List | Out-File artifacts/release-validation/1.2.1/sha256.txt
