...
- **手势操作**：列表中右滑完成或恢复任务，左滑把任务安排到明天；沿用安静的完成反馈，不弹提示条。两个方向的动作可在「设置 → 滑动手势」中分别改为完成或恢复、安排到明天或关闭。今天的事全部完成后，列表会为你庆祝。
...
截图来自 1.2.0 release 模拟器测试；新安装不含示例任务。

## 📦 安装

在 [Releases](https://github.com/shihuaidexianyu/todo/releases/latest) 下载 `todo-1.2.0-release.apk`，在 Android 10 或以上设备安装。AAB 用于分发构建，手机安装请选择 APK。

当前版本 **1.2.0**（versionCode **6**），沿用此前 release 签名，可覆盖升级。Debug 版签名不同，不能与 release 版互相覆盖安装（卸载会清除本地数据）。
...
1.2.0 已通过 19 项单元测试与 23 项模拟器端测试；Lint 无错误。覆盖范围见 [验证记录](docs/verification.md)。
