...
- **快速记录**：收件箱随手记，创建时一键设为今天截止，保存后直接回到列表；支持从其他应用分享文本新建任务，也可长按桌面图标快捷新建。
- **标签整理**：多标签分类，在清单页用标签筛选条按标签查看；收件箱只显示未安排且未打标签的任务。
- **任务信息**：行内显示日期、提醒和标签；有备注时显示标记与一行预览。
- **结束一天**：收起今天的列表，随时点击“重新展开今天”恢复。任务、日期与提醒保持原样；次日自动恢复显示。
- **搜索与已完成**：搜索只在清单页内，可切换「全部 / 已完成」，按标签筛选或搜索标题、备注、标签，点击勾选框恢复任务。
- **本地提醒**：系统通知提醒；无需账号，应用不申请网络权限。
- **完成反馈**：完成与恢复不弹提示条；勾选后淡出、列表平滑补位；可选轻微震动与清脆短音，静音或勿扰时不播放提示音。
- **手势操作**：列表中右滑完成或恢复任务，左滑把任务安排到今天；沿用安静的完成反馈，不弹提示条。两个方向的动作可在「设置 → 手势与反馈」中分别改为完成或恢复、安排到今天、安排到明天或关闭。今天的事全部完成后，列表会为你庆祝。
...
截图来自 1.2.1 release 模拟器测试；新安装不含示例任务。

## 📦 安装

在 [Releases](https://github.com/shihuaidexianyu/todo/releases/latest) 下载 `todo-1.2.1-release.apk`，在 Android 10 或以上设备安装。AAB 用于分发构建，手机安装请选择 APK。

当前版本 **1.2.1**（versionCode **7**），沿用此前 release 签名，可覆盖升级。Debug 版签名不同，不能与 release 版互相覆盖安装（卸载会清除本地数据）。
...
1.2.1 已通过 19 项单元测试与 24 项模拟器端测试；Lint 无错误。覆盖范围见 [验证记录](docs/verification.md)。
...
├── Motion.kt          动画设置
└── CompletionFeedback.kt 完成震动与提示音
```

[产品规格](todo-product-spec.md) · [UI 设计记录](docs/ui-research/design-notes.md)。原规格中的日终改期流程已由 1.1.2 的”仅收起今天”交互替代；底部导航已合并为「清单」页（搜索、标签与已完成都收进清单），收件箱改为只显示未打标签的任务。
