...
截图来自 1.1.0 release 模拟器测试；新安装不含示例任务。

## 📦 安装

在 [Releases](https://github.com/shihuaidexianyu/todo/releases/latest) 下载 `todo-1.1.0-release.apk`，在 Android 10 或以上设备安装。AAB 用于分发构建，手机安装请选择 APK。

当前版本 **1.1.0**（versionCode **3**），沿用此前 release 签名，可覆盖升级。Debug 版签名不同，迁移前请先在设置中导出备份。
...
1.1.0 已通过 28 项单元测试、6 项界面工作流测试及 release 安装回归；Lint 无错误。覆盖范围见 [验证记录](docs/verification.md)。
...
[产品规格](todo-product-spec.md) · [UI 设计记录](docs/ui-research/design-notes.md)。原规格中的日终改期流程已由 1.1.0 的“仅收起今天”交互替代。
